...
export class ProfileComponent implements OnInit {
  ...
  public saveUserDetails() {
    this.appService.editUser(this.userDetails.userId, this.userDetails).subscribe(
      response => {
        if (response.status == 200) {
          this.appService.setUserInfoInLocalStorage(this.userDetails);
          this.snackBar.open('Profile updated sucessfully', 'Close', { duration: 4000, });
  ...
  public recoverPassword() {
    this.appService.recoverPassword(this.userDetails.email).subscribe(
      response => {
        if (response.status == 200) {
          this.snackBar.open('Password reset email sent', 'Close', { duration: 4000, });
        }
      },
      error => {
        console.log(error)
  ...
  public deleteUser() {
    ...
    this.appService.deleteUser(this.userDetails.userId).subscribe(
      response => {
        if (response.status == 200) {
          this.snackBar.open('Your account has been deleted sucessfuly.', 'Close', { duration: 4000, });
          ...
          this.snackBar.open('Some error occured. Please refresh this page', 'Close', { duration: 4000, });
          console.log(response)
        }
      },
      error => {
        this.snackBar.open('Some error occured. Try again', 'Close', { duration: 4000, });
        console.log(error)
      }
    )
